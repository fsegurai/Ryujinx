...
namespace Ryujinx.UI.Common.Helper
{
    public static partial class FileAssociationHelper
    {
        ...
        [SupportedOSPlatform("linux")] private static readonly string _mimeDbPath =
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".local", "share", "mime");
        ...
        private static bool AreMimeTypesRegisteredLinux() =>
            File.Exists(Path.Combine(_mimeDbPath, "packages", "Ryujinx.xml"));
            ...
            if ((uninstall && AreMimeTypesRegisteredLinux()) || (!uninstall && !AreMimeTypesRegisteredLinux()))
            {
                ...
                if (mimeProcess.ExitCode != 0)
                {
                    Logger.Error?.PrintMsg(LogClass.Application,
                        $"Unable to {installKeyword} mime types. Make sure xdg-utils is installed. Process exited with code: {mimeProcess.ExitCode}");
                ...
                {
                    Logger.Error?.PrintMsg(LogClass.Application,
                        $"Could not update local mime database. Process exited with code: {updateMimeProcess.ExitCode}");
                }
            }
        ...
        private static bool AreMimeTypesRegisteredWindows()
        {
            ...
            static bool CheckRegistering(string ext)
            {
                ...
                if (openCmd is null) return false;

                string keyValue = (string)openCmd.GetValue(string.Empty);

                return keyValue is not null && (keyValue.Contains("Ryujinx") ||
                                                keyValue.Contains(AppDomain.CurrentDomain.FriendlyName));
            }
        }
