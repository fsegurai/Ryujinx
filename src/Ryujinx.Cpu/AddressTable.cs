    ...
    public unsafe class AddressTable<TEntry> : IAddressTable<TEntry> where TEntry : unmanaged
    {
        ...
        public AddressTable(AddressTableLevel[] levels, bool sparse)
        {
            ...
            if (sparse)
            {
                ...
                _sparseFill =
                    new MemoryBlock(268435456ul, MemoryAllocationFlags.Mirrorable); //low Power TC uses size: 65536ul
        ...
        public void SignalCodeRange(ulong address, ulong size)
        {
            ...
            _sparseBlockSize = Math.Max(_sparseBlockSize,
                BitUtils.AlignUp(entries, bottomLevelEntries) * (ulong)sizeof(TEntry));
        }
