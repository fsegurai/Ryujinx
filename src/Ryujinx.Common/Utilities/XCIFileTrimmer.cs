                    ...
                    try
                    {
                        ...
                        if (timedSw.Elapsed.TotalSeconds > 0)
                        {
                            Log?.Write(LogType.Info,
                                $"Checked at {readSizeB / (double)XCIFileTrimmer.BytesInAMegabyte / timedSw.Elapsed.TotalSeconds:N} Mb/sec");
                        }
        ...
        public OperationOutcome Untrim(CancellationToken? cancelToken = null)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        Log?.Write(LogType.Info,
                            $"Wrote at {bytesToWriteB / (double)XCIFileTrimmer.BytesInAMegabyte / timedSw.Elapsed.TotalSeconds:N} Mb/sec");
                    }
        ...
        private bool CheckAndReadHeader(bool assumeKeyArea)
        {
            ...
            if (_fileSizeB < 32 * 1024)
            {
                Log?.Write(LogType.Error,
                    "The source file doesn't look like an XCI file as the data size is too small");
                return false;
            }
            ...
            if (head != XCIFileTrimmer.HeaderMagicValue)
            {
                ...
                {
                    Log?.Write(LogType.Error,
                        "The source file doesn't look like an XCI file as the header is corrupted");
                }
            ...
            {
                Log?.Write(LogType.Error,
                    $"The source file doesn't look like an XCI file as the Cartridge Size is incorrect (0x{cartSizeId:X2})");
                return false;
            }
